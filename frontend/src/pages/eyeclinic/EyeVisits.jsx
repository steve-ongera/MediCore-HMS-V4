  ...
  if (loading && visits.length === 0) {
    return (
      <div className="loading-screen">
        ...
        <span className="loading-screen__label">Loading eye clinic visits...</span>
      </div>
    );
  }
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <h1 className="page-title">Eye Clinic Visits</h1>
          <p className="page-subtitle">Manage patient eye clinic visits</p>
        </div>
        <div className="page-header__actions">
          <button className="btn btn-secondary" onClick={load}>
            <i className="bi bi-arrow-clockwise  me-1"></i> Refresh
          </button>
          <Link to="/eyeclinic/register" className="btn btn-primary">
            <i className="bi bi-plus-circle  me-1"></i> Register Visit
          </Link>
        </div>
      </div>
        ...
        <div className="card-body p-0">
          {visits.length === 0 ? (
            <div className="empty-state">
              ...
              <h3 className="empty-state__title">No eye clinic visits found</h3>
              <p className="empty-state__desc">
                {search 
                  ? "No visits match your search criteria." 
                  : "Register a new eye clinic visit to get started."}
              </p>
              {!search && (
                <Link to="/eyeclinic/register" className="btn btn-primary">
                  <i className="bi bi-plus-circle  me-1"></i> Register Visit
                ...
                <tbody>
                  {visits.map((v) => (
                    <tr key={v.id}>
                      ...
                      <td>{v.ophthalmologist_name || "—"}</td>
                      <td>{v.chief_complaint || "—"}</td>
                      <td>{new Date(v.visit_date).toLocaleString()}</td>
                      <td className="cell-actions">
                        <Link to={`/eyeclinic/${v.id}`} className="btn btn-secondary btn-sm">
